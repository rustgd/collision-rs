    ...
    S: BaseFloat,
{
    ...
    fn generate_corners(half_dim: &Vector2<S>) -> Vec<Point3<S>> {
        vec![
...
impl<S> Primitive for RectangularPlane<S>
where
    S: BaseFloat,
{
    type Point = Point3<S>;
...
impl<S> ComputeBound<Aabb3<S>> for RectangularPlane<S>
where
    S: BaseFloat,
{
...
impl<S> ComputeBound<Sphere<S>> for RectangularPlane<S>
where
    S: BaseFloat,
{
...
impl<S> Discrete<Ray3<S>> for RectangularPlane<S>
where
    S: BaseFloat,
{
...
impl<S> Continuous<Ray3<S>> for RectangularPlane<S>
where
    S: BaseFloat,
{
